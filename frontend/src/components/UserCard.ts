...
import Router from "@/router/Router";
import { newChat, mockConversations, selectConversation } from "@/pages/chat";
...
class UserCard extends BaseAttributeValidationElement<UserCardAttributes> {
  ...
  static getAttributesValidators() {
    return super.defineValidator<UserCardAttributes>({
      ...
      "is-pending": {
        required: false,
        conditional: { variant: "possibleFriend" },
      },
      "has-invited-me": {
        required: false,
        conditional: { variant: "possibleFriend" },
  ...
  AuthManager.getInstance()
    ...
    .then((res) => {
      res?.json().then((data: { error?: string }) => {
        ...
        const playerCardEl: UserCard | null = document.querySelector(
...
const AddFriendHandler = (userId: string) => {
  AuthManager.getInstance()
    .authFetch(API.auth.friends.add, {
      method: "POST",
      body: JSON.stringify({ userId }),
    })
    .then((res) => {
...
const CancelFriendRequestHandler = (userId: string) => {
  AuthManager.getInstance()
    .authFetch(API.auth.friends.requests.pending.cancel, {
      method: "POST",
      body: JSON.stringify({ userId }),
    })
    .then((res) => {
  ...
  AuthManager.getInstance()
    ...
    .then((res) => {
      res?.json().then((data: { error?: string }) => {
        ...
        const card: UserCard | null = document.querySelector(
          `user-card#blocked-user-id-${userId}`,
        );
        if (card) {
          card.hidden = true;
        }
...
const getButtonAndHandleClick = (
  e: MouseEvent,
  classId: ButtonClassId,
...
document.addEventListener("click", (e) => {
  ...
  getButtonAndHandleClick(e, "message", (userId) => {
    ...
    // Create a target user object
    const targetUser = {
      id: parseInt(userId),
      username: card?.getAttribute("display-name") || "User",
      avatarUrl: card?.getAttribute("avatar-url") || "",
    };

    // Check if conversation already exists
    const existingConversation = mockConversations.find(
      (c) => c.id === targetUser.id && c.isPrivate,
    );

    if (!existingConversation) {
      // Only create a new conversation if it doesn't exist
      newChat(loggedInUser, targetUser);
    }
    ...
    const conversation = mockConversations.find((c) => c.id === targetUser.id);

    // If the conversation exists, select it
    if (conversation) {
      setTimeout(() => {
        selectConversation(targetUser.id, conversation, loggedInUser);
