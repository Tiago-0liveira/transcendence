		...
		<div class="user flex-1">
			...
			<main class="max-w-[90%]">
				<div class="flex flex-col">
					...
					<user-card variant="profile" user-id="${user.id}" avatar-url="${user.avatarUrl}" display-name="${user.displayName}" ></user-card>
				</div>
				<div class="flex flex-col mt-5">
					<div class="flex gap-4 items-center">
						<button id="FetchMore" class="${fetchMoreButtonDynamicClasses.join(" ")} rounded-md bg-blue-500 p-1 text-white">Fetch More</button>
					</div>
					<h3 class="text-2xl self-start font-bold mb-1">Friends</h3>
					<ul class="flex flex-col gap-2 mt-2" id="friends-list">
