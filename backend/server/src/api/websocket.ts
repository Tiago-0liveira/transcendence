...
export const websocketHandler = async (fastifyInstance: FastifyInstance) => {
	fastifyInstance.get<{
		...
		socket.on('message', socketOnMessage(fastifyInstance, socket, clientContext));
...
const socketOnMessage = (fastifyInstance: FastifyInstance, socket: WebSocket, clientContext: ClientThis) => async function (rawMessage: RawData) {
	...
	try {
		...
		if (elapsedTime > 10) {
			console.warn(`Long-running message processing: ${elapsedTime} ms, messageType: ${parsedMessage.type}`)
		}
...
const socketOnClose = (fastifyInstance: FastifyInstance, socket: WebSocket, clientContext: ClientThis) => async function (rawMessage: RawData) {
	...
	for (const [roomId, room] of activeGameRooms) {
		...
		room.brackets.forEach(async bracket => {
			if (bracket.game && (bracket.lPlayer === clientContext.userId || bracket.rPlayer === clientContext.userId)) {
