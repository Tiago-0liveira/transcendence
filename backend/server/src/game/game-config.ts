...
import { gameRoomFuncs } from "./game-room"
...
export const handleNewGameConfig = async function (clientContext: ClientThis, message: SelectSocketMessage<"new-game-config">) {
	...
	activeGameRooms.set(nameHash, {
		...
		lastUpdate: Date.now(),
		requiredPlayers: message.playersNumber,
		...
		playerJoined: gameRoomFuncs.playerJoined,
		playerLeft: gameRoomFuncs.playerLeft,
		playerReadyChanged: gameRoomFuncs.playerReadyChanged,
		getPlayer: gameRoomFuncs.getPlayer
	})
	console.log(`room created: ${message.roomName}`)
	return clientContext.socket.send(JSON.stringify({
		type: "join-game-room",
		roomId: nameHash
	} satisfies SelectSocketMessage<"join-game-room">))
}

export const handleGameRoomGetData = async function (clientContext: ClientThis, message: SelectSocketMessage<"game-room-join-request">) {
	...
	room.playerJoined(clientContext.userId, dbUser.result.displayName)
...
export const handleDeleteGameRoom = async function (clientContext: ClientThis, message: SelectSocketMessage<"delete-game-room">): Promise<boolean> {
	const room = activeGameRooms.get(message.roomId)
	if (!room) {
	...
	room.connectedPlayers.forEach(player => {
		connectedSocketClients.get(player.id)?.socket?.send(JSON.stringify({ 
			type: "leave-game-lobby", 
			roomId: room.id,
			reason: `Game room ${room.name} has been deleted by the owner.`
		} satisfies SelectSocketMessage<"leave-game-lobby">))
	})
